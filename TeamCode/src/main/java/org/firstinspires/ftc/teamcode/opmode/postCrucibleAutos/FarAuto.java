...
public abstract class FarAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Action driveToCollectAndShootFirst = autoPositions.collectAndShoot(Alliance.RED, 1, false,
                isRed ? AutoPositions.redFarShootingPose : AutoPositions.blueFarShootingPosition,
                Math.PI, Math.PI * 0.5,
                isRed ? AutoPositions.redFirstLine : AutoPositions.blueFirstLine, 53,
                -Math.PI * 0.5, 0, new double[] {3, 3, 3},
                robotStatus);

        Action driveToCollectAndShootSecond = autoPositions.collectAndShoot(Alliance.RED, 2, customizableParams.openGateOnSecond,
                isRed ? AutoPositions.redFarShootingPose : AutoPositions.blueFarShootingPosition,
                Math.PI, Math.PI * 0.5,
                isRed ? AutoPositions.redSecondLine : AutoPositions.blueSecondLine,
                62, -Math.PI * 0.5, 0, new double[] {3, 3, 3},
                robotStatus);

        Action driveToCollectAndShootLoadingZone = isRed ?
                autoPositions.redCollectAndShootLoadingZone(false, false) :
                autoPositions.blueCollectAndShootLoadingZone(false, false);

        Action driveToCollectAndShootThird = autoPositions.collectAndShoot(Alliance.RED,3, customizableParams.openGateOnThird,
                isRed ? AutoPositions.redFarShootingPose : AutoPositions.blueFarShootingPosition,
                Math.PI, Math.PI * 0.5,
