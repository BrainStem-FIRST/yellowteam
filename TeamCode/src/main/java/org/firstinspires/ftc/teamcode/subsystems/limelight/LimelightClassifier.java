...
public class LimelightClassifier extends LLParent {
    ...
    public boolean inValidClassifierRegion;
    public double[] classifierDetectionOutput;
    ...
    public void update() {
        ...
        inputs[0] = robot.alliance == Alliance.RED ? 1 : 0; // red or blue alliance
        inputs[1] = inCloseZone(robotPose) ? 1 : 0; // close or far zone
        inputs[2] = 72 - Math.abs(getCameraY()); // distance from classifier
...
/*
last working python snapscript code
import cv2
import numpy as np
import math
